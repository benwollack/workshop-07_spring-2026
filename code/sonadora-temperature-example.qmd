...
The sonadora_clean data frame has the information for all plots (plot name, elevation, daily temperature), with one observation based on the date.

The sonadora_sum takes all information from one plot and finds the mean temperature at that plot. Elevation information is retained.
...
The temperature is dropping as elevation increases. It appears linear.
...
Run `temperature_model` is the console.
...
Point 16 is an outlier and appears to have a nonlinear pattern associated. We may have to remove this point for analysis, as the model may only be linear over a small range (200-900m)

Otherwise, the linear model seems to fit here.
...
**For each meter of elevation gain, you would expect a decrease in temperature equivalent to 0.006 ± 0.0003 (SE) °C.**
...
# making a summary table for the model for the rendered pdf
